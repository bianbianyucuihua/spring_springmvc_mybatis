...
<body class="hold-transition skin-purple sidebar-mini">
			...
			<section class="content">
						...
						<div class="table-box">
								...
								<div class="form-group form-inline">
									<div class="btn-group">
										<button type="button" class="btn btn-default" title="新建"
											onclick="location.href='${bathpath}/order/add'">
							...
							<table id="dataList"
								...
								<tbody>
									...
									<c:forEach items="${pageInfo.records}" var="orders">
										...
										<tr>
											...
											<td class="text-center">
												...
												<button type="button" class="btn bg-olive btn-xs" onclick="location.href='${bathpath}/order/findById?id=${orders.id}'">详情</button>
                ...
                <div class="box-footer">
                    ...
                    <div class="box-tools pull-right">
						<ul class="pagination">
							<li>
								<a href="${bathpath}/order/findAll?current=1&size=${pageInfo.size}" aria-label="next">
									<span aria-hidden="true">首页</span>
								</a>
							</li>
							<c:if test="${pageInfo.current!=1}">
								<li>
									<a href="${bathpath}/order/findAll?current=${pageInfo.current-1}&size=${pageInfo.size}" aria-label="Previous">
							...
							<c:forEach var="i" begin="${begin}" end="${end}">
								<li ${i==pageInfo.current?"class='active'":""}>
									<a href="${bathpath}/order/findAll?current=${i}&size=${pageInfo.size}">${i}</a>
								</li>
							</c:forEach>
							<c:if test="${pageInfo.current!=pages}">
								<li>
									<a href="${bathpath}/order/findAll?current=${pageInfo.current+1}&size=${pageInfo.size}" aria-label="next">
										<span aria-hidden="true">下一页</span>
									</a>
								</li>
							</c:if>
							<li>
								<a href="${bathpath}/order/findAll?current=${pages}&size=${pageInfo.size}" aria-label="next">
	...
	<script>
		function changePageSize() {
			...
			//向服务器发送请求，改变没页显示条数
			location.href = "${bathpath}/static/orders/findAll.do?page=1&pageSize="
					+ pageSize;
		}
